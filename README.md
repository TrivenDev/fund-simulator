...
- 策略参数区：收益排名前 N、最多持有数量、同板块限制、持有周期、单只仓位上限。
...
5. 买入时遵守以下约束：
   ...
   - 单只基金默认最高占账户资金 `60%`。
6. 买入后默认持有约 `1` 个月。
7. 到下一个轮动月，先卖出到期持仓，再基于新的月度排名重新买入。
8. 模拟结束时强制卖出剩余持仓，并计算最终账户资产。
...
建议首次测试时先把“最多抓取”设为较小值，例如 `50` 或 `100`，确认流程正常后再提高到 `1000`。
...
- 增加交易决策解释：展示当月排名前 N、入选原因和被排除原因。
- 增加 CSV/HTML/PDF 回测报告导出。
- 增加多策略对比，例如动量轮动、低回撤优先、风险平价、定投策略。
- 把前端策略引擎从 `app.js` 中进一步拆分，便于测试和复用。
...
AKShare 全市场基金目录可能有上万条记录。逐只抓取历史净值会耗费较长时间，也可能触发接口限流。当前系统把最大抓取数量限制为 `1000`，是为了保证本地演示、开发和调试的可控性。

### 如果我把前端数量设置成 2000 会怎样？

后端会把它限制到 `1000`。前端也会做同样限制，避免创建过大的抓取任务。
